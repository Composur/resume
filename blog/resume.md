...
3. 对重构的页面进行优化，React路由切割按需加载等。
4. 系统优化，缩短了首屏加载时间由10.4s到3.66s
##### 知识图谱移动端
1. 负责移动端项目二次开发
2. 优化应用性能，配置`Nginx`，使用缓存，减少`HTTP`请求，提升访问速度，优化`webpack`构建，使打包静态资源加上md5后缀来命中缓存。
#####  企业级智能CRM
1. 服务端渲染项目，期间利用缓存减少HTTP请求次数`fn.cacheObj[fn.tadId]=res`。解决复杂的业务交互，一个小模块中存在大量的业务层级嵌套，使用`art-template`解决。
...
1.  网络故障处理、linux服务器运维，Shell脚本编写、VLAN划分、无线网覆盖等、设备为Cisco

## 技能清单(使用过)
+ 前端开发：HTML5/JavaScript(es6)/CSS3//HTTP
+ 后端开发：阅读过NodeJS源码，如`os`模块等。
